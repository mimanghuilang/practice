...
(function (root, factory) {
    ...
    function photoClip(container, option) {
        ...
        function initClip() {
            ...
            canvas.setAttribute("width", option.outputSize[0]);
            canvas.setAttribute("height", option.outputSize[1]);
            $("body").append(canvas);
            canvas.style = "border: 1px solid #fff123;width:360px;height:360px";
        }
        ...
        function clipImg() {
            ...
            if (hisScale.scaleX === -1 && hisScale.scaleY === 1) {
                ...
                Imgarea = (Imgarea) % 4>0? (Imgarea) % 4- 1:3;
            }
            else if (hisScale.scaleX === -1 && hisScale.scaleY === -1) {
                Imgarea = (Imgarea) % 4 > 2 ? (Imgarea) % 4 - 2 : (Imgarea) % 4 + 2;
            }
            else {
                console.log("没变");
            }
            console.log(curX, curY, "当前的位置");
            console.log(local, "位置");
            ...
            switch (Imgarea) {
                case 0:
                    console.log("第一象限");
                    translateX = -(local.x / option.size[0]) * canvas.width;
                    translateY = -(local.y / option.size[1]) * canvas.height;
                    break;
                case 1:
                    console.log("第二象限");
                    translateX = canvas.width + (local.x / option.size[0]) * canvas.width;
                    translateY = -(local.y / option.size[1]) * canvas.height;
                    break;
                case 2:
                    console.log("第三象限");
                    translateX = canvas.width + (local.x / option.size[0]) * canvas.width;
                    translateY = canvas.height + (local.y / option.size[1]) * canvas.height;
                    break;
                case 3:
                    console.log("第四象限");
                    translateX = -(local.x / option.size[0]) * canvas.width;
            ...
            // 翻转加缩放
            ctx.scale(hisScale.scaleX * scale, hisScale.scaleY * scale);
            // 绘制图片
